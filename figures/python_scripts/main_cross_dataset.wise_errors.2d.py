...
descriptors_sd_list = torch.load(args.apex.format(args.seed))
...
p_apex = suffixes[0].cuda()
    
descriptors_sd_list = torch.load(args.a.format(args.seed))
if not type(descriptors_sd_list) is list:
    descriptors_sd_list = [descriptors_sd_list]
init_suffixes = []
for d in descriptors_sd_list:
...
p_a = init_suffixes[0].cuda()
    
descriptors_sd_list = torch.load(args.b.format(args.seed))
if not type(descriptors_sd_list) is list:
    descriptors_sd_list = [descriptors_sd_list]
init_suffixes = []
for d in descriptors_sd_list:
    ...
    assert 'suffix_vectors' in d
    init_suffixes.append(d['suffix_vectors'])
p_b = init_suffixes[0].cuda()

distance = (p_a - p_apex).square().sum().sqrt().item()
print('a-apex distance: {}'.format(distance))
distance = (p_b - p_apex).square().sum().sqrt().item()
print('b-apex distance: {}'.format(distance))
cos_sim = (
    F.normalize(
        (p_a - p_apex).view(-1).unsqueeze(0)
    ) * F.normalize(
        (p_b - p_apex).view(-1).unsqueeze(0)
    )
).sum().item()
...
for alpha in alphas:
    ...
    for beta in alphas:
        ...
        suffix = p_apex + (p_b-p_apex)*alpha + beta * (p_a-p_apex)
